...
# Merge min/max values into merged_df using PID_ (from min_max_df) and PID (in merged_df)
data <- df %>%
...
#data <- read.csv("/home/afrahkhairallah/Documents/Alex/Paper/06052025/14052025/min_max_tooutpatientmultuni_.csv")
...
df <- data %>%
  select(Neuts_Binom, Age_At_Enrolment.x,Sex_Bino1,SuppO2_Binom, NLR3_Binom, Comorbid_Binom, HIV_Binom,CD4LymphocytesAbs__min,Lymph_abs_min) %>%
  na.omit()
...
p_multi <- ggplot(multivariate_results_plot_df, aes(x = Condition, y = OR)) +
  ...
  scale_x_discrete(labels = custom_labels) +
  # Log-scaling y-axis
  # ✔️ Updated y-axis with non-scientific labels
  scale_y_continuous(
    trans = "log10",
...
## Save plot
ggsave("uniOddsRatios_categorical.png", plot = p_uni, width = 8.1, height = 7, dpi = 600)
ggsave("multiOddsRatios_categorical_.Non-sci.png", plot = p_multi, width = 8.1, height = 7, dpi = 600)
